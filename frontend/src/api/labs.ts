...
export const deleteDataset = (id: string) => api.delete(`/labs/datasets/${id}`).then(r => r.data)
...
export const uploadDataset = (name: string, task_type: string, files: File[]) => {
  const form = new FormData()
  form.append('name', name)
  form.append('task_type', task_type)
  files.forEach(f => form.append('files', f))
  return api.post('/labs/datasets/upload', form, { headers: { 'Content-Type': 'multipart/form-data' } }).then(r => r.data)
}
